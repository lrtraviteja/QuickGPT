  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (data.success) {
        ...
        setUser(prev => ({...prev, credits: prev.credits - (mode === 'text' ? 1 : 2)}))
      } else {
    ...
    m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      <div ref={containerRef} className='flex-1 mb-5 overflow-y-scroll'>
        ...
        {/* Three Dots Loading */}
        {
          loading && (
           ...
           bg-gray-500 dark:bg-white animate-bounce' ></div>
              <div className='w-1.5 h-1.5 rounded-full
           bg-gray-500 dark:bg-white animate-bounce' ></div>
              <div className='w-1.5 h-1.5 rounded-full
           bg-gray-500 dark:bg-white animate-bounce' ></div>
      ...
      {mode === 'image' && (
        <label className='inline-flex items-center gap-2 mb-3 text-sm mx-auto' >
          ...
          className='cursor-pointer' checked={isPublished}/>
        </label>
      )}

      {/* Prompt Input Box */}
      <form onSubmit={handleSubmit} className='flex items-center gap-4 bg-primary/20
      dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 
